...
def get_output(context):
	...
	return context.stdout_capture.getvalue() if hasattr(context, 'stdout_capture') else ""
...
def step_impl(context):
	...
	if "Error:" in output:
		...
		real_errors = [
		    e for e in errors_with_content
...
def step_impl(context):
	output = get_output(context)
	assert "Error:" in output or "Connection" in output or hasattr(context, 'error'), "No connection errors reported"
...
def step_impl_multiple_tasks(context, num_tasks):
	...
	for file_path in files:
		thread = threading.Thread(target=context.client.send_single_task, args=(file_path,))
...
def step_impl(context):
    ...
    assert context.total_time <= acceptable_time, f"Execution time {context.total_time:.2f}s exceeds the acceptable limit of {acceptable_time}s"
...
def step_impl(context):
    ...
    try:
        ...
        assert "New task after reinitialization" in result["output"], "Unexpected output"
...
def step_impl(context):
    ...
    assert "New task after reinitialization" in new_output, "New task results are missing"
    
    for i in range(3):
        assert f"Test task {i}" not in new_output, f"Previous task result {i} is still present"
...
def step_server_is_restarted(context):
	...
	try:
		...
		print(
		    f"WARNING: An error occurred during server restart: {e}. Skipping check."
		)
