...
def run_common_tests():
	...
	all_common_tests_result = subprocess.run([
		...
		"-s", os.path.join(current_dir, "common"), 
		"-p", "test_*.py"
	...
	try:
		...
		if not os.path.exists(features_dir):
			print(f"The directory '{features_dir}' does not exist. Skipping behavioral tests.")
			return 0 if success else 1
			
		feature_files = [f for f in os.listdir(features_dir) if f.endswith(".feature")]
		if not feature_files:
			print(f"No .feature files found in '{features_dir}'. Skipping behavioral tests.")
			return 0 if success else 1
