...
def run_server_tests():
    ...
    try:
        ...
        if not os.path.exists(features_dir):
            print(
                f"Directory '{features_dir}' does not exist. Skipping behavioral tests."
            )
        else:
            feature_files = [
            ...
            if not feature_files:
                print(
                    f"No .feature files in directory '{features_dir}'. Skipping behavioral tests."
                )
            else:
                print(
                    f"Found {len(feature_files)} .feature files for behavioral tests."
                )
                subprocess.run(
    ...
    try:
        ...
        security_features_dir = os.path.join(
            current_dir, "server", "security", "features"
        )

        if not os.path.exists(security_features_dir):
            print(
                f"Directory '{security_features_dir}' does not exist. Skipping security tests."
            )
        else:
            feature_files = [
                f for f in os.listdir(security_features_dir) if f.endswith(".feature")
            ]
            if not feature_files:
                print(
                    f"No .feature files in directory '{security_features_dir}'. Skipping security tests."
                )
            else:
