...
def step_sanitize_unsafe_elements(context):
    ...
    assert (
        "os.system" not in context.result["sanitized_code"]
    ), "Dangerous code was not removed"
...
def step_execute_sanitized_code(context):
    ...
    assert (
        "This is potentially dangerous" not in context.result["output"]
    ), "Dangerous code was executed"
...
def step_server_handles_all_tasks(context, total_tasks):
    ...
    assert (
        len(context.all_results) == total_tasks
    ), f"Expected {total_tasks} tasks, but got {len(context.all_results)}"
