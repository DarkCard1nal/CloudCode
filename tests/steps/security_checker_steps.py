...
def step_impl(context):
	try:
		...
		has_safe_subprocess = 'safe subprocess calls' in context.original_content or 'shell=False' in context.original_content
		...
		if hasattr(context, 'is_safe_subprocess_in_docker'):
			...
			for op in context.unsafe_operations:
				print(f"  - {op.get('type', 'unknown')}: {op.get('description', 'no description')}")
		...
		if hasattr(context, 'is_safe_subprocess_in_docker') and context.is_safe_subprocess_in_docker:
			print(f"Clearing unsafe operations in Docker mode. Found {len(context.unsafe_operations)} unsafe operations")
			context.unsafe_operations = []
			print(f"After clearing: {len(context.unsafe_operations)} unsafe operations remain")
		elif 'safe subprocess calls' in context.original_content or 'shell=False' in context.original_content:
			original_count = len(context.unsafe_operations)
			for op in context.unsafe_operations[:]:
				if op.get('type') == 'subprocess_dangerous_call' and 'shell=False' in op.get('content', ''):
					context.unsafe_operations.remove(op)
				elif op.get('type') == 'dangerous_import' and 'subprocess' in op.get('description', ''):
					context.unsafe_operations.remove(op)
			print(f"Regular mode: removed {original_count - len(context.unsafe_operations)} false positives")
...
def step_then_unsafe_imports_removed(context):
    ...
    for pattern in import_patterns[2:]:
        if re.search(pattern, context.original_content, re.MULTILINE):
            assert not re.search(pattern, content, re.MULTILINE), f"Unsafe import matching '{pattern}' was not removed"
...
def step_impl(context):
	...
	assert "subprocess.check_output" not in safe_content or "subprocess.check_output([^)]*shell\\s*=\\s*False" in safe_content, "System command execution 'subprocess.check_output' was not removed"
...
def step_impl(context):
	...
	assert "subprocess.check_output(cmd, shell=True)" not in safe_content, "Unsafe code 'subprocess.check_output' was not removed"
...
def step_impl(context, error_type):
	...
	if error_type == "file not found":
		...
		assert "size" in error_message or "too large" in error_message, f"Expected 'exceeds maximum allowed size' error, got: {error_message}"
	elif error_type == "encoding":
		assert "codec" in error_message or "decode" in error_message, f"Expected 'encoding' error, got: {error_message}"
	else:
...
def step_impl(context, issue_type):
	...
	if issue_type == "subprocess":
		assert "subprocess.check_output(command, shell=True)" not in safe_content, "Unsafe subprocess.check_output was not removed"
		assert "subprocess.Popen(command, shell=True)" not in safe_content, "Unsafe subprocess.Popen was not removed"
	elif issue_type == "socket":
		...
		if "download_content" in context.original_content:
			assert "download_content(url)" not in safe_content or "urllib.request.urlopen" not in safe_content, "Unsafe network access code was not removed"
		if "make_request" in context.original_content:
			assert "make_request(url)" not in safe_content or "requests.get" not in safe_content, "Unsafe network access code was not removed"
	elif issue_type == "pickle":
		...
		assert "pickle.load" not in safe_content, "Pickle deserialization was not removed"
	elif issue_type == "input validation":
		if "get_user_input" in context.original_content and "eval" in context.original_content:
			assert "eval(user_input)" not in safe_content, "Eval of user input was not removed"
...
def step_impl(context):
	...
	assert context.unsafe_operations, "File was incorrectly marked as safe"
	assert any("aws" in op["description"].lower() or "dangerous_variable" in op["type"] for op in context.unsafe_operations), "AWS credentials were not detected"
...
def step_impl(context):
	...
	assert "pk_test_" not in safe_content.lower(), "Stripe Publishable Key was not removed"
	assert "stripe.api_key" not in safe_content, "Stripe API key was not removed"
	assert "publishable_key" not in safe_content, "Stripe publishable key variable was not removed"
...
def step_then_code_obfuscation_detected(context):
	assert any(op['type'] == 'obfuscation' or 'obfuscated' in op['description'].lower() or 'base64' in op['description'].lower() 
			   for op in context.unsafe_operations), "Code obfuscation techniques were not detected"
	
	with open(context.safe_file_path, "r") as safe:
		safe_content = safe.read()
	assert "base64.b64decode" not in safe_content, "Base64 decode function was not removed"
	assert "base64.b64encode" not in safe_content, "Base64 encode function was not removed"
...
def step_then_insecure_crypto_detected(context):
	assert any(op['type'] == 'insecure_crypto' or 'crypto' in op['description'].lower() or 'md5' in op['content'].lower() or 'hazmat' in op['content'].lower()
			  for op in context.unsafe_operations), "Insecure cryptographic practices were not detected"
	...
	assert "cryptography.hazmat" not in safe_content, "Insecure cryptography modules were not removed"
...
def step_then_redos_detected(context):
	assert any(op['type'] == 'redos' or 'regex' in op['description'].lower() or 'catastrophic' in op['description'].lower() or 'backtracking' in op['description'].lower()
			  for op in context.unsafe_operations), "ReDOS vulnerabilities were not detected"
...
def step_then_template_injection_detected(context):
	assert any(op['type'] == 'template_injection' or 'template' in op['description'].lower() 
			  for op in context.unsafe_operations), "Template injection vulnerabilities were not detected"
	
	with open(context.safe_file_path, "r") as safe:
		safe_content = safe.read()
	assert "render_template_string" not in safe_content.lower(), "Template rendering from string was not removed"
...
def step_then_ssrf_detected(context):
    assert any(op['type'] == 'ssrf_attempt' for op in context.unsafe_operations), "SSRF vulnerabilities were not detected"
    ...
    for pattern in dangerous_patterns:
        if re.search(pattern, context.original_content, re.MULTILINE):
            assert not re.search(pattern, safe_content, re.MULTILINE), f"Dangerous SSRF pattern {pattern} was not removed"
    
    if "access_localhost" in context.original_content:
        assert "requests.get('http://localhost:8080/admin')" not in safe_content, "Access to localhost was not removed"
    
    if "access_internal_network" in context.original_content:
        assert "requests.get('http://10.0.0.1/admin')" not in safe_content, "Access to internal network was not removed"
...
def step_then_container_escape_detected(context):
    assert any(op['type'] == 'container_escape' for op in context.unsafe_operations), "Container escape attempts were not detected"
    
    with open(context.safe_file_path, "r") as safe:
        safe_content = safe.read()
    
    # Sensitive files /proc
    assert "/proc/self/environ" not in safe_content, "Access to /proc/self/environ was not removed"
    assert "/proc/self/cmdline" not in safe_content, "Access to /proc/self/cmdline was not removed"
    ...
    # Docker socket
    assert "/var/run/docker.sock" not in safe_content, "Access to Docker socket was not removed"
    ...
    assert "dirty_cow" not in safe_content, "dirty_cow exploit reference was not removed"
...
def step_impl(context):
    assert context.processing_time < 5.0, f"Processing time {context.processing_time} seconds is too long"
...
def step_impl(context):
    ...
    has_run = bool(re.search(r"subprocess\.run\(\[[^\]]+\],\s*shell=False", safe_content))
    has_check_output = bool(re.search(r"subprocess\.check_output\(\[[^\]]+\]", safe_content))
...
def step_impl(context):
    ...
    assert "os_like_function" in safe_content, "Safe function with 'os' in name was removed"
    assert "my_os_system" in safe_content, "Safe variable with 'system' in name was removed"
    assert "eval_string" in safe_content, "Safe variable with 'eval' in name was removed"
    assert "mock_subprocess" in safe_content, "Safe function with 'subprocess' in name was removed"
...
def step_impl(context):
    ...
    for op in context.unsafe_operations:
        if ('obfuscation' in op.get('type', '') or 
            'Base64' in op.get('description', '') or 
            'encoding' in op.get('description', '').lower() or
            'obfuscated' in op.get('description', '').lower()):
    ...
    assert context.original_content != safe_content, "Safe file is identical to the original"
    
    assert "unsafe" in safe_content.lower() or "removed" in safe_content.lower(), "No indicators of code modification found"
